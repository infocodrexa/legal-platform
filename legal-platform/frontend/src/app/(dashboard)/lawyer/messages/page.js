...
export default function LawyerMessagesPage() {
  const { user } = useAuth();
  const appointmentsQuery = useLawyerAppointments({ page: 1, limit: 50 });
  ...
  useEffect(() => {
    ...
    async function loadHistory() {
      ...
      try {
        ...
        // non-fatal, matches the client-side Messages page
      } finally {
  ...
  if (conversations.length === 0) {
    return (
      <div>
        <DashPageHeading title="Messages" description="Conversations with clients." />
        <EmptyState icon="MessageSquare" title="No conversations yet" description="Messages open up once you accept a consultation request." />
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 overflow-hidden rounded-card border border-paper-line bg-paper-raised md:grid-cols-[280px_1fr]">
        <div className="divide-y divide-paper-line border-b border-paper-line md:border-b-0 md:border-r">
          {conversations.map((c) => (
            ...
            >
              <span className="text-sm font-medium text-ink">{c.user?.name || "Client"}</span>
        ...
        <div className="flex h-[520px] flex-col">
          <div className="flex items-center justify-between border-b border-paper-line px-5 py-3.5">
            <p className="font-medium text-ink">{active?.user?.name || "Client"}</p>
          ...
          <div className="flex-1 space-y-3 overflow-y-auto p-5">
            ...
            {typingUserId && typingUserId !== user?.id && <p className="text-xs text-ink-muted">Typing…</p>}
          </div>
